...
import { convertURL } from "@/app/api/convert";
...
const Home: NextPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    const res = await convertURL(url.current);
    if (res) {
      addArticle({
        article: res.article,
        title: res.title,
        articleDuration: res.articleDuration,
      });
      router.push("/editor");
    }
  };
    ...
    <div className="py-20 px-5 md:px-16 flex flex-col items-center space-y-20">
      <section className="bg-primary w-full md:w-[85%] lg:w-[75%]  h-full space-y-14 flex flex-col items-center font-semibold p-14 rounded-xl text-secondary">
        ...
        <p className="font-normal text-gray-300 w-3/4 text-center">
          Convertissez vos articles en vidéos en un clic. Il suffit de copier le
          lien de l'article et de cliquer sur convertir. C'est aussi simple que
          ça. Essayez maintenant!
        </p>
      </section>
      <section className="w-full flex flex-wrap text-primary justify-between px-24">
        <div className="w-80">
          ...
          <p className="text-justify">
            ...
            expérience fluide, que vous utilisiez un smartphone, une tablette ou
            un ordinateur.
          </p>
        </div>
        ...
        <div className="w-80">
          ...
          <p className="pb-5 text-justify">
            ...
            génère une narration fluide et des visuels attrayants en un rien de
            temps.
          </p>
